-- Create admin system for phone-based authentication
...
-- Function to make a phone number admin
...
BEGIN
    ...
    ELSE
        -- Regular user (only if they don't already have a role)
        INSERT INTO public.user_roles (user_id, role)
        VALUES (NEW.id, 'user')
...
-- Function to list pending admin phone numbers
CREATE OR REPLACE FUNCTION public.get_pending_admins()
RETURNS TABLE(phone_number text, created_at timestamptz)
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
AS $$
BEGIN
    ...
    RETURN QUERY
    SELECT pa.phone_number, pa.created_at
